...
class ImagePreparation:
    def __init__(self, uploads_path, student_path, target_shape: tuple = None):
        self.uploads_path = uploads_path
        self.student_photos_path = student_path
    ...
    def get_photos_set(self, student_id, random_student_id, uploaded_file):
        images = self.load_image(student_id, random_student_id, self.uploads_path, self.student_photos_path, uploaded_file)
        x_arrays = self.preprocess_image(*images)

        return x_arrays

    @staticmethod
    def load_image(student_id, random_student_id, image_path, student_path_, uploaded_file):
        uploaded_img = Image.open(image_path + uploaded_file)

        student_path = path.join(student_path_, student_id + "\\")
        student_img = Image.open(
            student_path + choice([x for x in listdir(student_path)]))

        random_student_path = path.join(student_path_, random_student_id + "\\")
    ...
    def preprocess_image(self, uploaded_img: Image.Image, student_img: Image.Image, random_student_img: Image.Image):
        ...
        uploaded_arr = np.array(uploaded_img, dtype="uint8")
        student_arr = np.array(student_img, dtype="uint8")
        random_student_arr = np.array(random_student_img, dtype="uint8")
